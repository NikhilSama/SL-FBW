...
$post_count = 0;
	...
	function extract_event_data($events_data,$apptabs)
	{	
		...
		// if( !empty($events_data['paging']['next']) )
		// {	
		// 	//checking if the next link of the data exists
		// 	$link  = $events_data['paging']['next'];
		...
		// 		extract_event_data($data,$apptabs);
		// 	}
	...
	function extract_video_data($video_data,$apptabs)
	{	
		global $video_count;
		global $mobapp_id;
		global $videos;
		...
		if( !empty($video_data['videos']['data']) )
		{
			...
			{	
				...
				$video['apptab_id'] = $apptabs['Videos'];
			...
			//checking if the pagination link exists for the album
			if( !empty($vide_info['paging']['next']) )
			{
				//checking if the next link of the data exists
				$link  = $videos_data['paging']['next'];
				...
				{	
					//calling the same function until the next link contains data
					extract_video_data($data,$apptabs);
				}
			}
